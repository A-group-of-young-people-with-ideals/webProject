...
public class WebIATWS extends WebSocketListener {
			...
			try (FileInputStream fs = new FileInputStream(file)) {
				...
				// 发送音频
					...
					case StatusFirstFrame: // 第一帧音频status = 0
						...
						// business.addProperty("rlang", "zh-hk"); // zh-cn :简体中文（默认值）zh-hk
	...
	public void onMessage(WebSocket webSocket, String text) {
		...
		if (resp != null) {
			if (resp.getCode() != 0) {
				System.out
						.println("code=>" + resp.getCode() + " error=>" + resp.getMessage() + " sid=" + resp.getSid());
	...
	public static class Text {
		...
		public String toString() {
			return "Text{" + "bg=" + bg + ", ed=" + ed + ", ls=" + ls + ", sn=" + sn + ", text='" + text + '\''
					+ ", pgs=" + pgs + ", rg=" + Arrays.toString(rg) + ", deleted=" + deleted + ", vad="
					+ (vad == null ? "null" : vad.getAsJsonArray("ws").toString()) + '}';
		}
	}
